...
from src.governance.event_log import read_log
...
def _fetch(path: str) -> list[dict]:
    """Fetch a list resource from the seller server. Returns empty list if unreachable."""
    try:
        r = httpx.get(f"{SELLER_SERVER_URL}{path}", timeout=2.0)
...
def _fetch_agents() -> list[dict]:
    result = _fetch("/governance/data/agents")
    if not result:
        from src.identity.dnsid import list_registry
        return list_registry()
    return result

def _fetch_manifests() -> list[dict]:
    result = _fetch("/governance/data/manifests")
    if not result:
        from src.identity.seller_manifest import SELLER_MANIFESTS
        return list(SELLER_MANIFESTS.values())
    return result

def _fetch_intent_mandates() -> list[dict]:
    result = _fetch("/governance/data/intent-mandates")
    if not result:
        from src.identity.signed_mandate import INTENT_MANDATES
        return list(INTENT_MANDATES.values())
    return result

def _fetch_cart_mandates() -> list[dict]:
    result = _fetch("/governance/data/cart-mandates")
    if not result:
        from src.identity.signed_mandate import CART_MANDATES
        return list(CART_MANDATES.values())
    return result

def _fetch_signed_offers() -> list[dict]:
    result = _fetch("/governance/data/signed-offers")
    if not result:
...
async def get_intent_mandates():
    ...
    """
    mandates = _fetch_intent_mandates()
...
async def get_summary():
    ...
    agents    = _fetch_agents()
    manifests = _fetch_manifests()
    intents   = _fetch_intent_mandates()
    carts     = _fetch_cart_mandates()
    offers    = _fetch_signed_offers()
...
async def get_audit_trail(limit: int = 100):
    """
    Durable audit log read from logs/audit.jsonl.
    Persists across server restarts. Ordered by timestamp (oldest first).
    Each event: ts, layer, event, entity, operator, detail.
    """
