...
- **Civic agent infrastructure RFC (added June 24 2026, open for discussion):** Boston / MIT NANDA RFC "Public Infrastructure for Private Agents: An AI Framework for Every Massachusetts Resident" (Garces, Torres, Bhatia, Raskar). Same NANDA lineage as SupplyMind. Its proposed four layer civic agent stack maps directly onto SupplyMind's built phases (L1 MCP servers, L2 KYA certified seller, L3 delegated buyer mandates). SupplyMind is effectively a running proof of concept for what the RFC only proposes. One genuine gap: the RFC's proposed output provenance layer (attest what an agent decided and how, not just who it acts for) is a candidate Phase 18. Full mapping and open discussion threads in docs/civic_infrastructure_mapping.md. Pending decisions: public facing writeup, working group application (WG1 Identity / WG4 Governance), and Phase 18 provenance design.
